...
def load_daily_orders(path: str) -> pd.DataFrame:
    ...
    orders["is_pro"] = orders["courses"].apply(
        lambda cs: any("про" in c.lower() for c in cs)
    )
    orders["is_start"] = orders["courses"].apply(
        lambda cs: any("старт" in c.lower() for c in cs)
    )
    return orders


def diff_in_diff(
    orders: pd.DataFrame, pre_start: str, pre_end: str, treat_start: str, treat_end: str
) -> dict:
    ...
    daily_pro = (
        orders[orders["is_pro"]]
        .groupby("date")
        .size()
        .reindex(full_range, fill_value=0)
    )
    daily_start = (
        orders[orders["is_start"]]
        .groupby("date")
        .size()
        .reindex(full_range, fill_value=0)
    )
    ...
    return {
        "pro_pre": pro_pre,
        "pro_treat": pro_treat,
        "pro_delta": pro_treat - pro_pre,
        "start_pre": start_pre,
        "start_treat": start_treat,
        "start_delta": start_treat - start_pre,
...
def weekend_confound_range(orders: pd.DataFrame, spike_date: str) -> dict:
    ...
    weekday_baseline = d.loc[
        ~d["weekday"].isin(["Saturday", "Sunday"]), "orders"
    ].mean()
    ...
    return {
        "spike_orders": spike,
        "lower_bound": spike
        - weekend_baseline,  # если бы день вёл себя как обычный выходной
        "upper_bound": spike
        - weekday_baseline,  # если бы день вёл себя как обычный будний
    }


def required_sample_size(
    baseline_rate: float, relative_lift: float, alpha: float = 0.05, power: float = 0.8
) -> int:
...
def power_simulation(
    ...
    for _ in range(n_sims):
        ...
        se = (
            np.sqrt(p_pool * (1 - p_pool) * (2 / n_per_group))
            if 0 < p_pool < 1
            else 1e-9
        )
