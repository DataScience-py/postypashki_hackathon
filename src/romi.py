...
def romi_by_channel(
    ...
    orders: dict[str, int],
    margin: float = 1.0,
    estimated_channels: set[str] | None = None,
) -> list[ChannelResult]:
    ...
    for channel, rev in revenue.items():
        ...
        results.append(
            ChannelResult(
                ...
                romi=romi,
                cac=cac,
                cost_is_estimate=channel in estimated_channels,
            )
        )
    return results


def romi_attr_vs_inc(
...
if __name__ == "__main__":
    ...
    for r in romi_by_channel(
        revenue, cost, orders, margin=0.7, estimated_channels=estimated
    ):
        tag = " [оценка стоимости]" if r.cost_is_estimate else ""
        cac_str = "н/д" if r.cac is None else f"{r.cac:,.0f} ₽".replace(",", " ")
        print(
            f"  {r.channel:15s} ROMI(margin=0.7)={format_romi(r.romi):22s} "
            f"CAC={cac_str}{tag}"
        )

    print("\n=== ROMI_attr vs ROMI_inc (демо на синтетике) ===")
    result = romi_attr_vs_inc(
        attributed_revenue=150_000, incremental_revenue=95_000, cost=12_000, margin=0.7
    )
    ...
    print(
        f"  Разрыв (выручка, приписанная рекламе, но случившаяся бы и так): {gap_str} ₽"
    )
